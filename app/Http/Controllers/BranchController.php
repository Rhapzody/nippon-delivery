...
class BranchController extends Controller
{
    public function detail(Request $req){
        ...
        if ($check == 1) {
            $branches = Branch::with(['subDistrict'])->where('id', '=', $req->input('search'))->get();
        } elseif ($check == 2) {
            $branches = Branch::with(['subDistrict'])->where('name', 'like', '%' . $search . '%')->get();
        } else {
            $branches = Branch::with(['subDistrict'])->paginate(5);
            $is_search = false;
        }
